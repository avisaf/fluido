...
vec2 bilerp(sampler2D d, vec2 p)
{
    ...
    vec4 uv = ij / gridSize.xyxy; // normalize
...
void main()
{
    // normalize the frag position to a value [0, 1]
    vec2 uv = gl_FragCoord.xy / gridSize.xy;
    float scale = 1.0 / gridScale;

    // trace point back in time
    // subtract the traveled distance from the current position
    vec2 p = gl_FragCoord.xy - timestep * scale * texture2D(velocity, uv).xy;

    // let the fluid dissipate and interpolate between the old values to get the new
